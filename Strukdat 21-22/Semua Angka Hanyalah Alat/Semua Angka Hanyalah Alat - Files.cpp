...
#include <stdbool.h>
#include <stdio.h>
FILE *_input  =  fopen("input6.txt","r");
FILE *_output = fopen("output6.txt","w");
...
void slist_print(SinglyList *list){
	SListNode *temp = list->_head;
	while(temp){
		fprintf(_output,"%d",temp->data);
		if(temp->next)fprintf(_output,"-");
		temp = temp->next;
	}
	fprintf(_output,"\n");
}
...
nums is_palindrom(int angka, int basis){
	...
	output.angka = angka;
	output.basis = basis;
	slist_init(&output.data);
	if(angka == 0){
		slist_pushFront(&output.data, 0);
		fprintf(_output,"Ya\n");
	}
	else{
		...
		while(angka > 0){
			...
			slist_pushFront(&output.data, slist_front(&stack));
		...
		if(is_palindrom) fprintf(_output,"Ya\n");
		else fprintf(_output,"Tidak\n");
	}
	return output;
}
...
int main(int argc, char const *argv[]){
    ...
    nums temp;
    while(1){
    	fscanf(_input,"%d%d",&angka,&basis);
    	if(angka == -1){
    		if(basis == -1) break;
    		if(basis < 0 || basis >= history._size){
    			fprintf(_output,"Out of range\n");
			}
			else{
	    		temp = dArray_getAt(&history, basis);
	    		fprintf(_output,"%d %d -> ",temp.angka, temp.basis);
	    		slist_print(&temp.data);
			}
		}
    	else{
